...
async function OnSendWinNo() {
  ...
  //  //console.log(" no:" + RandomWinAmount);
  usersingleNoChoice = [];
  if (playerName != "") {
...
async function OnTimeUp() {
  ...
  if (j === 1) {
    ...
    if (playerData.length > 0) {
      ...
      console.log("📭  players this round.  Win-------:",Win_singleNo,winX,ROUND_COUNT,imagenumber,NewpreviousWin_single);
    ...
    //   console.log("📭 No players this round. Random Win-------:",winX,imagenumber,winNo,winNo,NewpreviousWin_single);
  ...
  var winNo = Math.floor(Math.random() * 12);
  var messagearray = ["Zero", "One", "Two", "Three", "Four", "Five", "Six",
  ...
  console.log("📭 No players this round. Random Win funtarget:", winNo);

  await game_runningfuntarget.create({
