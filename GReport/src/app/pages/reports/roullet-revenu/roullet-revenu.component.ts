...
@Component({
  selector: 'app-roullet-revenu',
  standalone: true,
  imports: [CommonModule, AccordianComponent, DialogComponent,NgxPaginationModule, FormsModule],
  templateUrl: './roullet-revenu.component.html',
  styleUrl: './roullet-revenu.component.scss'
})
export class RoulletRevenuComponent  {
...
totalRows=[]
  ...
  getList() {
    ...
    this.service.getRolletRevenu(filter).subscribe((d: any) => {
  ...
  getTotal() {
    ...
    let filter = this.getFilter();
    this.service.getRolletRevenueTotal(filter).subscribe((d: any) => {
