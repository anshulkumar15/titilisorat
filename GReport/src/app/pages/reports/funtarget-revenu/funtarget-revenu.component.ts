...
@Component({
  selector: 'app-funtarget-revenu',
  standalone: true,
  imports: [CommonModule, AccordianComponent, DialogComponent,NgxPaginationModule, FormsModule],
  templateUrl: './funtarget-revenu.component.html',
  styleUrl: './funtarget-revenu.component.scss'
})
export class FuntargetRevenuComponent {
...
totalRows=[]
  ...
  getList() {
    ...
    this.service.getFuntartgetRevenu(filter).subscribe((d: any) => {
  ...
  getTotal() {
    ...
    let filter = this.getFilter();
    this.service.getFuntargetRevenueTotal(filter).subscribe((d: any) => {
