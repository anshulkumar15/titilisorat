...
@Component({
  ...
  styleUrl: './weekly-details.component.scss'
})
export class WeeklyDetailsComponent {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe((users) => {
      ...
      // Don't auto-load data, wait for user to click accordion
    });
  }

 toggleAccordion(type: string) {
    if (this.activeAccordion === type) {
      ...
      if (type === 'gaming') {
        this.loadGamingReport(); // 👈 Load child profit when gaming accordion clicked
      } else {
  ...
  // Load child total profit (Gaming Report)
  loadGamingReport() {
    const payload = {
      email: this.auth.getEmail(),   // logged-in user email
      s_date: this.options.s_date,
      e_date: this.options.e_date
    };

    this.service.getChildTotalProfit(payload).subscribe({
      next: (res: any) => {
        this.gamingReport = res?.data || [];
      },
      error: (err) => {
        console.error("Error fetching child profit:", err);
        this.gamingReport = [];
      }
    });
  }

  // Existing methods for transfers/reports below
  loadAllReports() {
    if (this.activeAccordion) {
      if (this.activeAccordion === 'gaming') {
        this.loadGamingReport();
      } else {
        this.typeChanged(this.activeAccordion);
      }
    }
  }
  ...
  search(term: any): void {
    ...
    this.options.page = 1;
    // Only search if accordion is active
    if (this.activeAccordion) {
      this.getList();
    }
  }
