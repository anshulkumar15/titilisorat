...
const getPlayerData = async (req, res) => {
  ...
  try {
    ...
    Users.dataTables(filter).then(function (table) {
...
const gamerunningfuntarget = async (req, res) => {
  ...
  try {
    ...
    let select = { playedTime: 1, RoundCount: 1, playername: 1, winX: 1, Win_singleNo: 1, betpoint: 1, winpoint: 1 };
    ...
    let rec = await getDataTable(Game_runningfuntarget, req, { find, select });
    res.json(rec);
  } catch (error) {
    console.log("error", error);
    res.status(500).send("Database error");
...
const sendMainIdPoints = async (req, res) => {
  try {
    ...
    let checkEmailRows = await Users.find({ email: email, role_id: 4 });
...
const getAdminDashborad = async (req, res) => {
  try {
    ...
    // Aggregate betTotal and winTotal from game_runningfuntarget
